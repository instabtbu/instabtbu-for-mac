...
class Common: NSObject {
    ...
    static func feng(buf:[UInt8],cmd:Int)->[UInt8]{
        ...
        let crcUInt8s = [UInt8(cmd),UInt8(jiamiUInt8s.count&0xff),UInt8(jiamiUInt8s.count>>8)] + jiamiUInt8s
        let crc = getCRC16(crcUInt8s)
        let crcBytes = [UInt8(0x7E)] + crcUInt8s
        let crcBytes2 = crcBytes + [UInt8(crc&0xFF),UInt8(crc>>8),UInt8(0x7E)]
        return crcBytes2
    }
